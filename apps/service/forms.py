...
class SingleImageForm(forms.ModelForm):
    """
    Works with normal admin + nested inlines.
    Requires model has a field named `image` (URLField / ImageField).
    """
    ...
    image_folder = "uploads"  # override in child forms
    ...
    def __init__(self, *args, **kwargs):
        ...
        # IMPORTANT: prefix-safe field name for inline formsets
        delete_field_name = self.add_prefix("delete_image")
                ...
                <div style="display:flex; align-items:center; gap:15px;">
                    <div style='flex-shrink:0;'>
                        <label style="display:flex; align-items:center; gap:6px; cursor:pointer;">
                            <input type="radio" name="{delete_field_name}" value="{image_url}"
    ...
    def save(self, commit=True):
        ...
        to_delete = self.data.get(self.add_prefix("delete_image"))
        if to_delete:
            try:
        ...
        if file:
            old = getattr(instance, "image", None)
            if old:
                try:
                    delete_file_from_digital_ocean(old)
                except Exception:
                    pass
